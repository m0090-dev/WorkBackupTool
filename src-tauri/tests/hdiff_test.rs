...
fn test_hdiff_args_with_temp_files() {
    ...
    // 3. 引数生成ロジックのテスト (zstd)
    let args = hdiff::build_hdiffz_args(&old_p_str, &new_p_str, &diff_p_str, "zstd");
    ...
    // 4. 引数生成ロジックのテスト (none)
    let args_none = hdiff::build_hdiffz_args(&old_p_str, &new_p_str, &diff_p_str, "none");
    ...
    let patch_args = hdiff::build_hpatchz_args(&old_p_str, &diff_p_str, &restore_p_str);
    assert_eq!(
        patch_args,
